...
/* ─── Type config (shared with AdminTracking) ────────────────────────── */
const typeConfig = {
  login: {
...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN COMPONENT — SupervisorTracking
═══════════════════════════════════════════════════════════════════════════ */
const SupervisorTracking = () => {
    ...
    async (silent = false) => {
      ...
      try {
        ...
        const [mainRes, allRes] = await Promise.all([
          api.get(`/supervisor-activities/my-tracking?${qs}`),
          api.get(`/supervisor-activities/my-tracking?${qsAll}`),
        ]);
        ...
        console.error("[SupervisorTracking] fetch error:", err);
      } finally {
        setLoading(false);
      }
    },
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-purple-50/30 to-blue-50/20">
        <div className="text-center">
          ...
          <p className="text-slate-500">
            Loading supervisor activity analytics…
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/20 to-blue-50/10 p-4 md:p-6 space-y-5">
                ...
                <div className="flex items-center gap-4 mt-1.5 flex-wrap text-xs text-slate-400">
                  ...
                  {supervisor?.createdAt && (
                    <span className="flex items-center gap-1">
                      ...
                      Since: {fmtDate(supervisor.createdAt)}
                    </span>
                  )}
                </div>
              </div>
      ...
      <Card delay={0.24}>
        <div className="bg-gradient-to-r from-blue-500/10 via-cyan-500/10 to-teal-500/10 -m-5 p-5 rounded-t-2xl border-b-2 border-blue-200 mb-4">
          ...
          <p className="text-xs text-slate-600 mt-1 font-medium">
            In-depth analysis of supervisor panel usage per page
          </p>
        </div>
      ...
      {/* ── 7. Supervisor Actions (CRUD / Export / Settings) ─────────────── */}
      <Card delay={0.25}>
        <SectionHeader
          icon={Settings}
                    ...
                    >
                      ...
                      <td className="py-2 px-2">
                        <div className="flex items-center gap-1.5">
                          <div
                            className={`w-6 h-6 rounded-md flex items-center justify-center ${isActiveDevice ? "bg-violet-500 text-white" : "bg-blue-50 text-blue-500"}`}
                          >
                      ...
                      <td className="py-2 px-2">
                        <span
                          className={`inline-block px-1.5 py-0.5 rounded-full text-[10px] font-bold ${dev.isMobile ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}
                        >
                          {dev.deviceType ||
      ...
      <Card delay={0.29}>
        <div className="bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 -m-5 p-5 rounded-t-2xl border-b-2 border-indigo-200 mb-4">
          ...
          />
          <p className="text-xs text-slate-600 mt-1 font-medium">
            Complete supervisor panel journey from login to logout — with
            detailed page tracking and time spent analysis
          </p>
        </div>
        {sessionMap.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div id={`ssession-${i}`} className="space-y-2">
                      <div className="flex items-center gap-2 mb-3">
