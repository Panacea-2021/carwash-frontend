...
const NotificationTracking = () => {
  ...
  useEffect(() => {
    const loadCustomers = async () => {
      try {
        ...
        console.error(
          "Failed to load customers for notification tracking:",
          error,
        );
      }
    };
  ...
  const topCustomers = useMemo(
    () =>
      Array.isArray(stats?.perCustomer) ? stats.perCustomer.slice(0, 6) : [],
    [stats],
  );
    ...
    <div className="p-6 space-y-6">
      <div className="rounded-3xl border border-[#dde7ff] bg-white/95 p-5 shadow-[0_14px_42px_rgba(19,36,84,0.12)]">
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-6">
          ...
          <KpiTile
            icon={Users}
            label="Customers Opened"
            value={stats?.uniqueCustomersOpened || 0}
            tone="emerald"
          />
          <KpiTile
            icon={Activity}
          ...
          <div className="grid grid-cols-1 gap-2 p-3 md:grid-cols-2 xl:grid-cols-3">
            {topCustomers.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-300 bg-white px-3 py-6 text-center text-sm text-slate-500 md:col-span-2 xl:col-span-3">
                No customer engagement data available for selected filters.
              </div>
            ) : (
              topCustomers.map((item) => (
                <div
                  key={item.customerId || item.mobile || item.customerName}
