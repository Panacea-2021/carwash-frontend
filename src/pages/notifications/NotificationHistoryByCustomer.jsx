...
import { BarChart3, Filter, RefreshCw, Users } from "lucide-react";
...
const NotificationHistoryByCustomer = () => {
  ...
  const loadStats = useCallback(async () => {
    try {
      setLoading(true);
      const res = await customerNotificationService.getCampaignStats({
      ...
      console.error("Failed to load customer-wise stats:", error);
      toast.error("Failed to load customer-wise history");
    } finally {
      setLoading(false);
    }
  ...
  const rows = useMemo(
    () => (Array.isArray(stats?.perCustomer) ? stats.perCustomer : []),
    [stats],
  );
      ...
      <div className="rounded-3xl border border-[#dde7ff] bg-white/95 p-5 shadow-[0_14px_42px_rgba(19,36,84,0.12)]">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-extrabold text-[#13203a] flex items-center gap-2">
              ...
              Customer-wise Notification History
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              Measure engagement per customer with open-rate and timeline stats.
            </p>
          </div>
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <StatTile
            ...
            value={stats?.uniqueCustomersNotified || 0}
          />
        </div>
        ...
        <DataTable
          title="Per-Customer Engagement"
          columns={columns}
          data={rows}
          loading={loading}
          hideSearch={false}
          actionButton={
            <div className="hidden sm:flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">
              <BarChart3 className="h-4 w-4" />
              <span>
                {stats?.uniqueCustomersOpened || 0} customers have opened
              </span>
            </div>
          }
        />

        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-xs text-slate-500">
          Tip: use the table search box to find any customer by name or mobile.
        </div>
      </div>
    </div>
  );
...
export default NotificationHistoryByCustomer;
