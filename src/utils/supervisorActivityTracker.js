/**
 * Supervisor Panel Activity Tracker
 * Tracks supervisor usage of the panel and sends batches to the backend.
...
class SupervisorActivityTracker {
  ...
  initialize() {
    ...
    const existingSession = sessionStorage.getItem("supervisor_session_id");
    if (existingSession) {
      this._sessionId = existingSession;
    } else {
      this._sessionId = `sup_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      sessionStorage.setItem("supervisor_session_id", this._sessionId);
    }
    ...
    // Request exact GPS/WiFi geolocation
    this._startGeolocation();

    console.log(
      "[SupervisorTracker] ✅ Initialized | session:",
      this._sessionId,
    );
  }
  ...
  startNewSession() {
    ...
    sessionStorage.setItem("supervisor_session_id", this._sessionId);
    this._isIdle = false;
    console.log("[SupervisorTracker] 🔄 New session started:", this._sessionId);
  }
  ...
  _resetIdleTimer() {
    ...
    if (this._isIdle) {
      ...
      console.log(
        "[SupervisorTracker] ▶ User returned — new session:",
        this._sessionId,
      );
    }
  ...
  _onIdle() {
    ...
    sessionStorage.removeItem("supervisor_session_id");
    this._sessionId = null;

    console.log("[SupervisorTracker] ⏸ User idle 1hr — session ended");
  }

  /* ── Geolocation ── */
  _startGeolocation() {
    if (!navigator.geolocation) {
      console.warn("[SupervisorTracker] Geolocation not supported");
      return;
    }

    navigator.geolocation.getCurrentPosition(
      (pos) => this._onGeoSuccess(pos),
      (err) => console.warn("[SupervisorTracker] Geo error:", err.message),
  ...
  _onGeoSuccess(position) {
    ...
    console.log(
      `[SupervisorTracker] 📍 Location: ${latitude.toFixed(5)}, ${longitude.toFixed(5)} (±${Math.round(accuracy)}m)`,
    );
  ...
  async _reverseGeocode(lat, lng) {
    try {
      ...
      if (data && data.display_name) {
        ...
        console.log("[SupervisorTracker] 🏠 Address:", data.display_name);
      }
    } catch (err) {
      console.warn("[SupervisorTracker] Reverse geocode failed:", err.message);
    }
  }
  ...
  dispose() {
    ...
    this._pageEnteredAt = null;
    this._isIdle = false;
    console.log("[SupervisorTracker] Disposed");
  }
  ...
  async _flush() {
    ...
    try {
      await api.post("/supervisor-activities/batch", { activities: batch });
    } catch (err) {
      this._queue = [...batch, ...this._queue].slice(0, 200);
      console.warn("[SupervisorTracker] Flush failed:", err.message);
    } finally {
      this._isFlushing = false;
    }
  }
...
// Singleton
const supervisorTracker = new SupervisorActivityTracker();
